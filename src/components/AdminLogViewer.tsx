...
export const AdminLogViewer = () => {
  ...
  const { data: combatLogs, loading: combatLoading } = useData(
    DB_PATH.COMBAT_LOGS,
  );
  const { data: tradingLogs, loading: tradingLoading } = useData(
    DB_PATH.TRADING_LOGS,
  );
  ...
  const renderCombatLogs = () => {
    ...
    if (!combatLogs)
      return (
        <div className="text-center py-8 text-muted-foreground">無戰鬥記錄</div>
      );
  ...
  const renderTradingLogs = () => {
    ...
    if (!tradingLogs)
      return (
        <div className="text-center py-8 text-muted-foreground">無交易記錄</div>
      );
