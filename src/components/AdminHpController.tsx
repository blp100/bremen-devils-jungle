...
export const AdminHpController = ({ players }: AdminHpControllerProps) => {
  ...
  if (!selectedPlayerId) {
    return (
      <div className="space-y-4">
        <div className="text-sm text-muted-foreground text-center mb-4">選擇要調整血量的玩家</div>
  ...
  return <PlayerHpEditor player={player} onBack={() => setSelectedPlayerId(null)} />
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Button onClick={() => adjustHp(-5)} variant="destructive" className="min-h-[48px] text-base" size="lg">
        ...
        <Button onClick={() => adjustHp(-1)} variant="outline" className="min-h-[48px] text-base" size="lg">
          <Minus className="h-5 w-5 mr-1" />
          -1
        </Button>
        <Button onClick={() => adjustHp(1)} variant="outline" className="min-h-[48px] text-base" size="lg">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <Button onClick={updateHp} className="flex-1 min-h-[48px] text-base" size="lg">
          <Check className="h-5 w-5 mr-2" />
          更新血量
        </Button>
        <Button variant="outline" onClick={onBack} className="flex-1 sm:flex-none min-h-[48px] text-base" size="lg">
